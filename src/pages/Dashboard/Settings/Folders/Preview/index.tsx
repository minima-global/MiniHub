...
export function Preview({ display, data, dismiss }: PreviewProps) {
  const { setActiveFolderTheme } = useContext(appContext);
  ...
  const setFolderBackground = () => {
    set('FOLDER_BACKGROUND', data);
    setActiveFolderTheme(data);
      ...
      <div className="flex flex-col h-full">
        ...
        >
          ...
          <div className="flex items-end flex-grow left-0 bottom-0 w-full">
            <Button onClick={setFolderBackground} variant={hasSetRecently ? 'secondary' : 'primary'}>
              {hasSetRecently && 'Folders background updated'}
              <div
              ...
              {!hasSetRecently && 'Set folders background'}
            </Button>
          </div>
        </div>
      </div>
