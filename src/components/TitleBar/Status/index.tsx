...
const Status = () => {
  ...
  const handleOnMouseLeave = () => setTimeout(() => {
    setHover(false)
  }, 1000);
  ...
  return (
    <div
      ...
      className={`rounded-full flex items-center bg-status-red h-[26px] px-2.5 text-sm font-bold text-black transition-all ${
        display ? 'scale-100 opacity-100' : 'scale-0 opacity-0'
      }`}
    >
      {hover ? <div className="ml-0.5 mr-2 transition-all ease-in-out">Node unlocked</div> : <div />}
  ...
  return (
    <div
      onMouseEnter={handleOnMouseEnter}
      onMouseLeave={handleOnMouseLeave}
      className={`flex items-center bg-status-red py-1.5 px-3 text-sm font-bold text-black transition-all ${
        display ? 'scale-100 opacity-100' : 'scale-0 opacity-0'
      } ${!hover ? 'rounded-full' : 'rounded-tr-full rounded-br-full'}`}
    >
      <div
        className={`absolute py-[2.5px] text-left pl-3 lg:block transition-all origin-right h-[26px] w-[120px] bg-status-red rounded-tl-full rounded-bl-full right-7 flex items-center opacity-100  ${
