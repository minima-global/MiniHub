const OnboardingModal: React.FC<React.PropsWithChildren<{ display: boolean, width?: string }>> = ({ display, children, width }) => {
    return (
        <div className={`w-full h-full flex items-center justify-center transition-all duration-[400ms] ${display ? 'opacity-100' : 'pointer-events-none opacity-0 translate-y-2'}`}>
